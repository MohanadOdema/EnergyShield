    ...
    def __init__(self, input_states, input_detections, taken_actions, action_space, scope_name,
        ...
        with tf.variable_scope(scope_name):
            ...
            if vf_hidden_sizes is None:
                ...
                self.vf = build_mlp(concat_input_states, hidden_sizes=vf_hidden_sizes, activation=tf.nn.relu, output_activation=tf.nn.relu)
...
class PPO_CASCADE():
    ...
    def __init__(self, input_state_shape, input_detections_shape, action_space,
        ...
        # Create placeholders
        self.input_states  = tf.placeholder(shape=(None, *input_state_shape), dtype=tf.float32, name="input_state_placeholder")
        self.input_detections = tf.placeholder(shape=(None, *input_detections_shape), dtype=tf.float32, name="input_detections_placeholder")
        ...
        self.policy        = PolicyGraph(self.input_states, self.input_detections, self.taken_actions, action_space, "policy", initial_std=initial_std)
        self.policy_old    = PolicyGraph(self.input_states, self.input_detections, self.taken_actions, action_space, "policy_old", initial_std=initial_std)
        ...
        summaries.append(tf.summary.histogram("train_step/input_detections", self.input_detections))
    ...
    def train(self, input_states, input_detections, taken_actions, returns, advantage):
    ...
    def predict(self, input_states, input_detections, greedy=False, write_to_summary=False):
        ...
        sampled_action, value, summaries, step_idx = \
            self.sess.run([action, self.policy.value, self.stepwise_prediction_summaries, self.predict_step_counter.var],
                feed_dict={self.input_states: input_states, self.input_detections: input_detections}
            )
    ...
    def update_old_policy(self):
        self.sess.run(self.update_op) # old_policy <-- current one
