...
if [ ! -d "$SCRIPT_DIR/paper_results/raw_data/casc_agent_1" ]; then
    ...
    RESULTSBIND=""
    DUMMYCONTAINER=`docker run --rm -d $HOSTNETWORK --label server=${SERVER} --mount type=volume,dst=/home/carla/EnergyShield/models,volume-driver=local,volume-opt=type=none,volume-opt=o=bind,volume-opt=device="${SCRIPT_DIR}/paper_results/raw_data" energyshield:${localuser} $user -d $SERVER $CORES $PORTNUM $CARLAPORT $MPIHOSTS ""`
    ...
    for CONT in $DCONTAINERS; do
      if [ ${DUMMYCONTAINER:0:12} = $CONT ]; then
        ...
        echo "Raw data extraction complete. Removing raw_data extraction container: $CONT"
      fi
    done
fi
