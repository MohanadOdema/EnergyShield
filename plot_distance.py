...
parser.add_argument("--plot_all", action='store_true', default=False, help='plot all plots in one figure')
...
for safety in safety_filter:
    for noise in gaussian_noise:
        load_data_path = "./plot_data/distance/Hist_Shield2_"+sup_string2+"_Safety_"+str(safety)+"_noise_"+str(noise)+".csv"
        df = pd.read_csv(load_data_path)
...
if not params['no_save']:
    plt.savefig('./plot_data/distance/off_belay_' +str(params['off_belay']) + '_combined_' + str(params['plot_all']) + '.svg', bbox_inches='tight')
plt.show()
