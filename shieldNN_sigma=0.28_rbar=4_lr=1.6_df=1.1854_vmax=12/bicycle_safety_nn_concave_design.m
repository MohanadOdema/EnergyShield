...
% Bicycle parameters:
lr = 1.6; % total vehicle length = 2*lr = 0.46 
% NOTE: THIS IS THE ~MAX VEHICLE LENGTH FOR SUBSEQUENT PARAMETERS!!
%   To allow a longer vehicle you may:
%       Increase \sigma;
%       INcrease rBar; or 
%       INcrease deltaFMax
deltaFMax = pi/4+0.4; % maximum steering angle: pi/4 = 45 degrees
betaMax = atan(0.5*tan(deltaFMax));
vmax = 18; % Maximum linear velocity

% Barrier function parameters:
rBar = 4; % Mimum physical distance to origin
sigma = 0.28; % Scaling factor that determines minimum dist. to 
...
radiusThresholds = [0 0.2 0.4 freeControlThreshold];
