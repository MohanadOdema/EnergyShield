...
offload_policy = ['Shield2_early', 'Shield2_belay']
...
for policy in offload_policy: 
    for safety in safety_filter:
        for noise in gaussian_noise:
            ...
            episodes_path = os.path.join("./models", params['model_name'], "experiments", "obs_"+str(params['len_obs'])+"_route_"+str(params['len_route']), str(params['map'])+"_ResNet152_"+policy+q_string+phi_string, 
                                        "PX2_"+str(params['deadline'])+"_Safety_"+safety+"_noise_"+noise, "plots")
            episode_summary_path = os.path.join("./models", params['model_name'], "experiments", "obs_"+str(params['len_obs'])+"_route_"+str(params['len_route']), str(params['map'])+"_ResNet152_"+policy+q_string+phi_string, 
